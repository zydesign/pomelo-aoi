...
// （aoiEventManager处理‘add’事件时，如果加入对象为怪物，会执行onMobAdd函数，除了通知灯塔点的玩家观察者，
//   还调用该函数，获取怪物看到的玩家类型ids，添加仇恨，加入ai攻击玩家）
 ...
 */
//(场景删除实体时，area.removeEntity会调用该函数)（玩家attack怪物致死亡；玩家pickItem拾取道具，玩家退出或切换场景）....................
...
//【】角色移动后，作为对象更新，需要做2件事：1.让新旧灯塔点添加删除对象。 2.通知新旧灯塔点的观察者们，看见与不看见这个对象

//【move动作发起更新对象，让Timer.updateObject调用该函数,推送消息给玩家群，让玩家看到和看不到该角色】.......................
pro.updateObject = function(obj, oldPos, newPos){
  ...
  if(p1.x===p2.x && p1.y===p2.y)
    return true;
	  ...
	  //观察者的删除组、添加组，在aoiEventManager响应‘update’事件时创建
...
//(场景删除实体时，area.removeEntity会调用该函数)（玩家attack怪物致死亡）....................................
...
//【】角色移动后，作为观察者被更新，需要做2件事： 1.让添加塔、删除塔添加删除该观察者。2.推送消息给自己，看见与看不见的对象组

//【move动作发起，让timer.updateWatcher更新观察者，并且推送消息给移动的自己，看见和不看见哪些对象群】
...
//通过坐标，观察者类型，获取某灯塔点指定类型的观察者（Timer.getWatcherUids和Timer.prototype.getWatchers调用该函数）
pro.getWatchers = function(pos, types){
	//如果坐标合法，转换坐标为灯塔点，获取某灯塔点指定类型的观察者
	if(this.checkPos(pos)){
    var p = this.transPos(pos);
