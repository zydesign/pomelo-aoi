...
var TowerAOI = function(config){
  ...
  this.rangeLimit = 5 || config.limit;      //范围限制
...
//初始化是根据地图大小和单灯塔大小，建立灯塔组（area实例aoi服务时，立即执行init函数）------------------------1
pro.init = function(){
	//计算得到灯塔列阵max灯塔点（不是坐标），即灯塔组右下角的灯塔点。左上为原点灯塔点为{0,0}
  this.max = {
...
pro.getIdsByRange = function(pos, range, types){
  ...
  var result = {};
  var p = this.transPos(pos); //通过对象的坐标，算出的对应的灯塔点
