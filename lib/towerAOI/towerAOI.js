...
//更新观察者，通过新旧坐标算出的灯塔范围矩形交集，让删除塔删除观察者，让增加塔组增加观察者-----------------------'updateWatcher'事件
...
pro.updateWatcher = function(watcher, oldPos, newPos, oldRange, newRange){
  ...
  if(p1.x===p2.x && p1.y===p2.y && oldRange===newRange)
    ...
    var addObjs = [];     //用于储存添加了观察者的灯塔所储存的对象组
    var removeObjs = [];  //用于储存删除了观察者的灯塔所储存的对象组
    ...
    for(i = 0; i < removeTowers.length; i++){
	  ...
	  //发射'updateWatcher'事件。为了推送消息给自己，更改部分实体为看不见，和看见新实体。（玩家移动会更新周围的可视对象）
