...
//通过指定的坐标、灯塔范围、对象类型组，获取指定范围内的实体id图阵, 单例是result[i][j] = entityId--------------------------对象组
 ...
 * 通过指定坐标、灯塔范围，获取范围内的对象id数组（不分类型），每一个元素为一个灯塔点的ids---------------------------------------对象数组
 */
pro.getIdsByPos = function(pos, range){
  ...
  var limit = getPosLimit(p, range, this.max);  //算出灯塔范围对角线端的2坐标{start: start, end :end}
...
//更新观察者，通过新旧坐标算出的灯塔范围矩形交集，计算出删除的被观察对象id，需要增加的被观察对象id
